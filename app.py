...
2. Fake review detection via a supervised text detector (Stage 2)
...
DEMO_MODE = not (
    ...
    Path("models/saved/fake_review_detector.pkl").exists()
)
...
def load_models():
    ...
    if not DEMO_MODE:
        try:
            ...
            detector_model = FakeReviewDetector.load_model(
                'models/saved/fake_review_detector.pkl'
            )
            threshold = detector_model.metadata.get('threshold', 0.5)
            
            # Load vocabulary
            preprocessor.load_vocabulary('models/saved/vocabulary.pkl')
            
            return sentiment_model, detector_model, preprocessor, threshold
...
def analyze_review(
    ...
    if DEMO_MODE:
        ...
        return {
            ...
            'detector_score': demo['detector_score'],
    ...
    # Stage 2: Supervised fake-review detection
    detector_score = detector_model.predict_fake_probability(clean_review)
...
def page_analyze_review():
    ...
    if analyze_button and review_text.strip():
        ...
        with tab2:
            st.text(review_text)
            st.info("In production mode, this view reflects the Stage 2 fake-review detector score.")
...
def page_model_performance():
    ...
    if Path('models/saved/fake_review_detector.pkl').exists():
        try:
            detector_model = FakeReviewDetector.load_model('models/saved/fake_review_detector.pkl')
            autoencoder_log = detector_model.metadata
        except Exception:
            autoencoder_log = None

    if autoencoder_log is None:
        try:
            with open('models/saved/training_log_autoencoder.json') as f:
                autoencoder_log = json.load(f)
        except:
            st.warning("Stage 2 training logs not found. Run training first.")
            autoencoder_log = None
...
def page_about():
    ...
    ReviewGuard is a two-stage review analysis system for detecting fake product reviews:
    ...
    ### Stage 2: Supervised Fake Review Detector
    - **Architecture**: Hybrid word + character TF-IDF with a linear classifier
    - **Training Data**: Labeled genuine/fake review pairs
    - **Task**: Predict fake-review probability directly
    - **Output**: Detector score + threshold-based classification
    ...
    - This signal, fused with the detector score, creates a more robust detector
    
    ### Score Fusion
    Final fake probability = 0.65 × detector_score + 0.35 × contradiction_score
